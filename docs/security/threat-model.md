...
| B2 | Authenticated **member** REST API | Trust (session cookie **or** personal API token) | `session()` `session.js`, guards `routes/guards.js:26-64` | Fails closed outside dev (401/503). A bearer token (`api_tokens`, issue #152) resolves to the same users row as a cookie and is checked first in every mode; a request that sends one never falls back to the cookie or dev header. |
...
| **Member via API token** | `Authorization: Bearer kuhn_…` (personal API token, issue #152) | Exactly the minting member's authority at the time of the call; **cannot** mint, list or revoke tokens (`routes/auth.js` refuses token-authenticated `/api/me/tokens`) | `session.js` `readBearerToken`, `db/api-tokens.js`, `routes/auth.test.js` |
...
| **Personal API tokens** | **Secret** | User | `api_tokens` — **sha256 only**, raw value returned once at mint (`db/api-tokens.js`) | **Required** | Required | User-revocable (account menu); expiry ≤ 1 year; deleted with the user (`ON DELETE CASCADE`) | **Raw token must never log** — mint/revoke log ids and names only; rejected tokens log path/method only | Never | Never | Never |
...
| **Provider/model configuration** | Internal | Deployment + org | `agents.model`, `config.js`; org **model profiles** and per-role routes (`model_profiles`, `agent_model_routes`, `db/model-profiles.js`, issue #107) hold provider/model/endpoint/capabilities and a credential *reference* (an `org_secrets` name), never a value | Standard | Required | Owner-managed (Settings → Models); deployment profiles read-only | Profile slugs/endpoints in audit meta; no secret material | Endpoint identity only (the profile decides egress) | n/a | n/a |
...
### 4.2 Policy per provider mode (implemented by issue #107/#111/#112 for the first three rows)
...
| **Organization BYOK** | Org-scoped secret (`org_secrets`), referenced by an org **model profile** by name, resolved server-side into the adapter constructor only (`agents/model-routing.js` `resolveCredential`); never returned to the browser | Only that org's content flows to that org's provider/credential | **Hard**: profiles and secrets are looked up by `(org_id, name)`, so another org's credential/profile cannot be named at all (`db/model-profiles.test.js`, `routes/model-profiles.test.js`) |
| **Custom OpenAI-compatible endpoint** | Org profile (`provider: openai-compatible`), credential optional for keyless local servers | Only to the profile's validated base URL (`validateBaseUrl`): absolute http(s), no credentials/query/fragment, **https required for public hosts**, loopback/link-local/private/metadata hosts refused unless the operator sets `KUHN_ALLOW_PRIVATE_MODEL_ENDPOINTS=true` (default on only in dev auth mode) | Endpoint is part of the profile, not model-selectable; the model chooses what to say, never where it goes |
| **Self-hosted / local model** | Local, possibly no credential — the OpenAI-compatible profile above with a private base URL | Egress stays on-prem; still an explicit, operator-enabled profile | Same |
...
| ID | Threat | Sev | Attacker / prereq | Impact | Current control | Evidence | Residual | Issue |
|---|---|---|---|---|---|---|---|---|
| **T-21** | **Uncontrolled provider spend / no concurrency ceiling** — no per-user/org/global limit on concurrent runs or dispatches | **High** | One authenticated editor | Open N SSE task streams; each tree burns ≤2.5 M weighted tokens; unbounded provider bill | Per-run token budget (2.5 M ×1.1) + 50-turn cap + depth ≤2; **org owners can set per-member and per-project budgets per UTC day/week/month (issue #110): the remaining allowance caps each run and an exhausted one refuses to start; usage is a durable per-job ledger (`jobs.weighted_tokens`)** | `config.js`, `db/org-budgets.js`, `agents/runtime.js` | **Partially fixed** — org budgets are opt-in (default unlimited); no org-wide or global ceiling and no concurrency cap, so N parallel runs can each spend up to the remaining allowance before the ledger catches up | **STH-19**, #110 |
...
| **T-28** | **Stale/uncontrolled jobs after suspension/removal** — org suspension does **not** kill in-flight agent runs; parked `ask_user` runs live indefinitely with unbounded event buffers | High | Suspended tenant with a run in flight; or any run parked on a question | Suspended tenant keeps mutating files + spending budget until the run ends; memory growth from parked runs | Suspension and membership removal flag every open job of the tenant (`jobs.cancel_requested_at`, `agents/tenancy.js`) and abort the live ones; the run gate (`runtime.js createRunGate`) re-checks tenancy at every control point (before a provider turn, before a mutating tool, on a question's wake), so a run stops within one tool call of the revocation. Parked runs still have no TTL. | `agents/tenancy.js`, `runtime.js createRunGate`, `tools/registry.js gated` | **Partially fixed** (#118 stage 1; parked-run TTL with stage 2) | #118 |
| **T-29** | **Provider outage / retry storm** — 5-attempt exponential backoff per turn; resume-on-retry can re-stream a half-completed turn | Medium | Provider degradation | Amplified load; duplicated partial output | Full-jitter backoff, cap 30 s; transient classification; wall-clock deadline per run tree (`jobs.deadline_at`, `AGENT_RUN_MAX_MS`, default 2 h) cancels with reason `deadline` and a hand-off note | `runtime.js createRunGate`, `config.js agent.runMaxMs` | No circuit breaker (#118 stage 5) | #118 |
