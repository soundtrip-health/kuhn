...
|---|---|---|---|---|
| TB-1 | **Web ↔ worker: durable event/control seam** | Cross-process via shared SQLite | Ordered append-only job/domain event rows (worker→web); persisted `ask_user` replies, cancellation, suspension flags (web→worker); optional local wakeup that correctness never depends on (ADR 002 §2.3) | This is job events, question/reply delivery, and Yjs/reviewer-room invalidation becoming **integrity/authorization-bearing state**: event rows drive what the web shows and closes, control rows drive what the worker executes. Replay must be idempotent; mutating tool calls have an **exactly-once logical effect** via stable operation identity, durable operation state/results, and reconciliation on lease reclaim — not cross-store atomicity (ADR 002 §2.4); a reclaimed worker never blindly reruns a mutating operation. Audit/idempotency: PLA-243. |
| TB-2 | **Web/worker ↔ sandbox service** | Local RPC (Unix socket/loopback) | Enumerated operation types carrying logical project/org/document identifiers, workspace-relative paths, and bounded options — never absolute host paths, mount specs, image names, Docker argv, or arbitrary commands; web calls it synchronously for render/export, worker for ingestion (ADR 002 §6) | The web and worker processes hold **no Docker access**; only the sandbox service touches the daemon (closes T-25). **The service enforces mount/path containment independently of its caller**: it canonicalizes against configured data roots, rejects `..`/symlink/absolute-path escapes, and constructs all mounts, images, and commands internally — a compromised web/worker process can invoke an allowed capability but cannot obtain arbitrary Docker control or host-filesystem mounts. Distinct least-privilege service identity where practical. PLA-240. |
...
| **Provider credentials** (`ANTHROPIC_API_KEY`, SMTP URL) | **Secret** | Deployment | `agent-backend/.env` only | **Required** (host) | **Never in the data backup — reissued/rotated at restore; escrowed only if not reissuable** (ADR 002 §8.3) | Manual | **Never** (code never sees them) | Never | Never | Never |
...
| **Data backup** | Confidential | Deployment (all tenants) | *Not built* — PLA-242; online-backup DB destination + `data/files/` + `data/orgs/` + recorded app/schema version (ADR 002 §8.1–8.2). **Contains no secrets** | **Required (encrypted)** | — | Generational retention to define | Never | Never | Never | The DR artifact (with the matching app artifact + escrow) |
| **Secret escrow** | **Secret** | Deployment | *Not built* — PLA-242; **separate security domain** (ADR 002 §8.3): `KUHN_SESSION_SECRET`, any future master key; SMTP cred only if not reissuable. Never co-located with the data backup | **Required + independently protected** | — | Rotate on incident/restore | Never | Never | Never | Never |
