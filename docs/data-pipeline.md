...
| Agent writes | `write_file` / `edit_file` / `move_file` tools (`agents/runtime.js`) | Same storage module, same limits; logged to the activity feed with the acting agent. Agent writes to `draft/**` — and to any **existing** file outside the agent-private `pm/` and `analyst/` folders (STH-44) — do **not** touch the file: they land as pending suggestions (`pending_edits` table, story 008-001) the PI reviews per-hunk in the editor; only acceptance writes the file, activity-logged and version-committed under the originating agent/job — "an AI edited my manuscript" becomes "I approved these changes". The seeding pipeline's first draft is the deliberate exception (direct write). |
...
- **Render & export** (`render.js`, `sandbox.js`): markdown → Typst → PDF
  ...
  export converts any markdown). All of it runs in Docker with
