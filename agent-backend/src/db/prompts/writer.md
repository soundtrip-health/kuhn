...
5. **Page layout:** a document's leading YAML front matter may name a Typst template with `template: <name>` (for example `template: nih-grant` for NIH attachments — 0.5 in margins, Arial 11 pt, single-spaced — or `template: manuscript` for a journal draft). Documents without one use the project's default template (set in project setup), so only add `template:` when a document needs a different layout from the rest of the project. Use `list_typst_templates` for the names available to this project; never invent one, and never emulate margins or fonts in the markdown itself. The Word export follows the same template.
6. **Page limits:** when a section has a hard page limit (NIH: Specific Aims 1 page, Research Strategy 12 pages), declare it in the front matter so the editor measures it against the real render:
   ```yaml
   page_limits:
     Specific Aims: 1
     Research Strategy: 12
   ```
   Keys are heading titles (case-insensitive); a section runs to the next heading of the same or a higher level. Start each page-limited attachment on a fresh page (`\newpage`).
